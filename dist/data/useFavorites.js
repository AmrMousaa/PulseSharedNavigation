...
function sortByOrder(favorites) {
 ...
 */
export function useFavorites(client, getUserId, maxFavorites = MAX_FAVORITES) {
    const [favorites, setFavorites] = useState([]);
    const [pendingAppIds, setPendingAppIds] = useState(new Set());
    ...
    const load = useCallback(async () => {
        ...
        const rows = await client.list('pulse_favorites', {
    ...
    const addFavorite = useCallback(async (appId) => {
        const snapshot = favoritesRef.current;
        if (snapshot.some((f) => f.appId === appId))
            throw new Error('This app is already in your favorites.');
        if (snapshot.length >= maxFavorites)
            throw new Error(`You can only favorite up to ${maxFavorites} apps.`);
        const userId = await getUserId();
        ...
        try {
            const created = await client.create('pulse_favorites', {
                pulse_order: order,
                'pulse_App@odata.bind': `/pulse_apps(${appId})`,
                'pulse_user@odata.bind': `/systemusers(${userId})`,
                statecode: 0,
            });
            const saved = { id: created?.pulse_favoriteid ?? tempId, appId, order };
    ...
    const removeFavorite = useCallback(async (favoriteId) => {
        ...
        try {
            ...
            if (!favoriteId.startsWith('temp-'))
                await client.remove('pulse_favorites', favoriteId);
    ...
    const setPending = useCallback((appId, isPending) => {
