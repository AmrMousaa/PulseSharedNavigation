 ...
 */
export function PulseNavigation({ client, getUserContext, maxFavorites, trackUsage, homeUrl = PULSE_HUB_URL, onGoHome, onLaunchApp, disableFavorites, onError, userName, powerAppId, currentAppId, ...sidebarProps }) {
    injectStyles();
    const nav = usePulseNavigation({ client, getUserContext, maxFavorites, trackUsage });
    const detectedAppId = useMemo(() => {
        if (currentAppId || !powerAppId)
            return currentAppId;
